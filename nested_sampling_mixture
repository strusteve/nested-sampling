...
def log_likelihood(data, gen_func,  all_params, scale):

    # Given Data
    xi, yi, yisig = data[0], data[1], data[2]

    func_params = all_params[3:]
    p_b, y_b, v_b = all_params[:3]

    # Likelihood of belonging to the 'good' gaussian
    li_good = scale * (1-p_b) * (1/(np.sqrt(2*np.pi*(yisig**2)))) * np.exp((-(yi - gen_func(xi, func_params))**2) / (2*(yisig**2)))
    li_bad = scale * p_b * (1/(np.sqrt(2*np.pi*(v_b + yisig**2)))) * np.exp((-(yi - y_b)**2) / (2*(v_b + yisig**2)))

    log_li = np.log(li_good+li_bad)
...
def run_nested_sampling(data, gen_func, N, prior_low, prior_high, scale):
    ...
    ytest = gen_func(xtest, best_fit[3:])
    ax1.plot(xtest, ytest, color='red')

    title = ' f(x) = '
    for i in range(3, len(best_fit)):
...
N = 10000
prior_low = [0, 0, 0, 0, 0, -5]
prior_high = [1, 10, 10, 10, 10, 5]
scale=100
